...
stress-apparmor.o: apparmor-data.h

#
#  generate apparmor data using minimal core utils tools from apparmor
#  parser output
#
apparmor-data.h: usr.bin.pulseaudio.eg config.h
	...
	$(PRE_V)echo "static const char g_apparmor_data[]= { " >> apparmor-data.h
	$(PRE_V)od -tx1 -An -v < apparmor-data.bin | \
		sed 's/[0-9a-f][0-9a-f]/0x&,/g' | \
		sed '$$ s/.$$//' >> apparmor-data.h
	$(PRE_V)echo "};" >> apparmor-data.h
	$(PRE_V)rm -f apparmor-data.bin
	$(PRE_V)echo "static const size_t g_apparmor_data_len = sizeof(g_apparmor_data);" >> apparmor-data.h
