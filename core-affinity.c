	...
	int *setbits)
{
	...
	sets = (cpu_set_t *)calloc((size_t)max_cpus, sizeof(*sets));
	if (!sets) {
	...
	while ((d = readdir(dir)) != NULL) {
		...
		char str[1024], *ptr, *token, *saveptr;
		cpu_set_t newset;
		int lo, hi;
		...
		CPU_ZERO(&newset);
		saveptr = NULL;
		for (ptr = str; (token = strtok_r(ptr, ",", &saveptr)) != NULL; ptr = NULL) {
