...
static void stress_ensure_positive(const char *const str)
{
	...
	for (ptr = str; *ptr; ptr++) {
		...
		if (isdigit((unsigned char)*ptr)) {
			...
			(void)fprintf(stderr, "invalid negative value %s\n", str);
			longjmp(g_error_env, 1);
			stress_no_return();
		}
	}
...
uint8_t stress_get_uint8(const char *const str)
{
	...
	if (val > UINT8_MAX) {
		(void)fprintf(stderr, "invalid value %s too large (> %d)\n", str, (int)UINT8_MAX);
		longjmp(g_error_env, 1);
		stress_no_return();
	}
	return (uint8_t)val;
...
uint64_t stress_get_uint64_byte(const char *const str)
{
	...
	if (strcasecmp(str, "LLC")  == 0) {
		...
		errno = 0;
		if ((sscanf(str + 1, "%d", &cache_level) != 1) || (errno != 0))
			cache_level = 0;
		...
		stress_cpu_cache_level_size_get((uint16_t)cache_level, &llc_size,
	...
	const char *const errmsg)
{
	...
	if ((len > 1) && (str[len - 1] == '%')) {
		...
		errno = 0;
		if ((sscanf(str, "%lf", &val) != 1) || (errno != 0)) {
...
int stress_parse_opt(const char *stressor_name, const char *opt_arg, const stress_opt_t *opt)
{
	...
	case TYPE_ID_UINT8:
		...
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_UINT8, &setting.u.uint8);
	case TYPE_ID_INT8:
		setting.u.int8 = stress_get_int8(opt_arg);
		stress_check_signed_range(opt_name, (int64_t)setting.u.int8, (int64_t)min, (int64_t)max);
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_INT8, &setting.u.int8);
	case TYPE_ID_UINT16:
		setting.u.uint16 = stress_get_uint16(opt_arg);
		stress_check_range(opt_name, (uint64_t)setting.u.uint16, min, max);
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_UINT16, &setting.u.uint16);
	case TYPE_ID_INT16:
		...
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_INT16, &setting.u.int16);
	case TYPE_ID_UINT32:
	...
	case TYPE_ID_SSIZE_T:
		...
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_SSIZE_T, &setting.u.ssize);
	case TYPE_ID_UINT:
		setting.u.uint = stress_get_uint(opt_arg);
		stress_check_range(opt_name, (uint64_t)setting.u.uint, min, max);
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_UINT, &setting.u.uint);
	case TYPE_ID_INT:
		setting.u.sint = stress_get_int(opt_arg);
		stress_check_signed_range(opt_name, (int64_t)setting.u.sint, (int64_t)min, (int64_t)max);
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_INT, &setting.u.sint);
	case TYPE_ID_INT_DOMAIN:
		...
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_UINT, &setting.u.sint);
	...
	case TYPE_ID_OFF_T:
		...
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_OFF_T, &setting.u.off);
	case TYPE_ID_STR:
		return stress_setting_set(stressor_name, opt_name, TYPE_ID_STR, opt_arg);
	case TYPE_ID_BOOL:
		if (!opt_arg)
	...
	case TYPE_ID_CALLBACK:
		...
		if (type_id != TYPE_ID_UNDEFINED)
			return stress_setting_set(stressor_name, opt_name, type_id, &setting.u);
		return EXIT_SUCCESS;
