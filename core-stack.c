...
/*
 *  stress_min_aux_sig_stack_size_get()
 ...
 */
static inline long int stress_min_aux_sig_stack_size_get(void)
{
#if defined(HAVE_SYS_AUXV_H) && \
...
/*
 *  stress_stack_sigstksz()
 *	wrapper for STRESS_SIGSTKSZ, try and find
 *	stack size required
 */
size_t stress_stack_sigstksz(void)
{
	...
	min = stress_min_aux_sig_stack_size_get();
#if defined(_SC_SIGSTKSZ)
...
/*
 *  stress_stack_minsigstksz()
 *	wrapper for STRESS_MINSIGSTKSZ
 */
size_t stress_stack_minsigstksz(void)
{
	static long int sz = -1;
	long int min;
