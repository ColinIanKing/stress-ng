...
static int OPTIMIZE3 stress_bitonicsort(stress_args_t *args)
{
	...
	stress_memory_anon_name_set(data, data_size, "bitonicsort-data");
	...
	if (stress_signal_handler(args->name, SIGALRM, stress_bitonicsort_handler, &old_action) < 0) {
...
tidy:
	...
	stress_metrics_set(args, "bitonicsort comparisons per sec",
		rate, STRESS_METRIC_HARMONIC_MEAN);
	rate = (sorted > 0.0) ? count / sorted : 0.0;
	stress_metrics_set(args, "bitonicsort comparisons per item",
		rate, STRESS_METRIC_HARMONIC_MEAN);
