...
static void *stress_close_func(void *arg)
{
	...
	if (!stress_setting_get("close-fds", &close_fds)) {
...
static int stress_close(stress_args_t *args)
{
    ...
    defined(HAVE_SHM_UNLINK)
		...
		if (dt > max_duration) {
			max_duration = dt;
			/* max delay is 75% of the max duration in microseconds */
...
tidy:
	...
	stress_metrics_set(args, "close calls per sec", rate, STRESS_METRIC_HARMONIC_MEAN);
