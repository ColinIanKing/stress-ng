	...
	int (*compare)(const void *p1, const void *p2))
{
	...
	register const ssize_t n = nmemb - 1;
	register void *ptr;
	...
	while (fib0 > 1) {
		register const ssize_t m = offset + fib2;
		register const ssize_t idx = (m < n) ? m : n;
		register int cmp;
...
static int OPTIMIZE3 stress_fibsearch(stress_args_t *args)
{
	...
	stress_metrics_set(args, "fibsearch comparisons per sec",
		rate, STRESS_METRIC_HARMONIC_MEAN);
	rate = (sorted > 0.0) ? count / sorted : 0.0;
	stress_metrics_set(args, "fibsearch comparisons per item",
		rate, STRESS_METRIC_HARMONIC_MEAN);
