...
static int stress_filehole(stress_args_t *args)
{
	...
	zero_buf = (uint64_t *)(((uint8_t *)buf) + page_size);
	...
	extents = (extents_count > 0.0) ? extents_total / extents_count : extents;
	stress_metrics_set(args, 0, "extents per file",
		(double)extents, STRESS_METRIC_GEOMETRIC_MEAN);
	stress_metrics_set(args, 1, "Mbytes per file (maximum)",
		max_size / (double)MB, STRESS_METRIC_GEOMETRIC_MEAN);
	stress_metrics_set(args, 2, "blocks used per file (maximum)",
