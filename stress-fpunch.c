	...
	while (ptr < ptr_end) {
		if (*ptr) {
			pr_inf("%s: data at file offset 0x%" PRIxMAX " was 0x%2.2x and not zero\n",
				args->name, (uintmax_t)(offset + (ptr - data)), (unsigned int)(*ptr & 0xff));
			return -1;
		}
		ptr++;
	}
...
static int stress_fpunch(stress_args_t *args)
{
	...
	if (s_pids == MAP_FAILED) {
			...
			args->name, STRESS_PUNCH_PIDS, stress_memory_free_get());
	...
	stress_memory_anon_name_set(buf, sizeof(*buf), "fpunch-buffer");
