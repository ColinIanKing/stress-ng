...
static const stress_help_t help[] = {
	...
	{ NULL,	"gpu-size N",		"specify upload texture SIZExSIZE" },
	{ NULL,	"gpu-upload N",		"specify upload texture N times per frame" },
	{ NULL,	"gpu-xsize N",		"specify framebuffer size x" },
	{ NULL,	"gpu-ysize N",		"specify framebuffer size y" },
	{ NULL,	NULL,		NULL }
};
...
{
	return stress_set_gpu_gl(opt, "gpu-size", INT_MAX);
}
...
static GLuint compile_shader(const char *text, int size, GLenum type)
{
	...
	if (!compiled) {
		...
		if (infoLen > 1) {
			...
			(void)fprintf(stderr, "Error compiling shader:\n%s\n", infoLog);
			free(infoLog);
		}
...
void load_shaders(void)
{
	...
	if((vertexShader = compile_shader(vert_shader, sizeof(vert_shader), GL_VERTEX_SHADER)) == 0)
		(void)fprintf(stderr, "Failed to compile vertex shader\n");
	if((fragmentShader = compile_shader(frag_shader, sizeof(frag_shader), GL_FRAGMENT_SHADER)) == 0)
		(void)fprintf(stderr, "Failed to compile fragment shader\n");
	if((program = glCreateProgram()) == 0)
		(void)fprintf(stderr, "Error creating the shader program\n");
	...
	if (!linked) {
		...
		if (infoLen > 1) {
			...
			(void)fprintf(stderr, "Error linking program:\n%s\n", infoLog);
			free(infoLog);
		}
...
void gles2_init(uint32_t width, uint32_t height, int frag_n, GLsizei texsize)
{
	...
	if(glGetError() != GL_NO_ERROR)
		(void)fprintf(stderr, "Failed to get the storage location of %d\n", frag_n);
	...
	if (texsize > 0) {
		...
		if(texsize > maxsize)
			(void)fprintf(stderr, "Image size exceeds max texture size \n");
...
EGLConfig get_config(void)
{
	...
	if(eglGetConfigs(display, NULL, 0, &num_configs) == EGL_FALSE)
		(void)fprintf(stderr, "There are no EGL configs \n");

	EGLConfig *configs = malloc(num_configs * sizeof(EGLConfig));
	if((eglChooseConfig(display, egl_config_attribs,
				configs, num_configs,
				&num_configs) == EGL_FALSE) || (num_configs == 0))
		(void)fprintf(stderr, "can't choose EGL config \n");

	for (int i = 0; i < num_configs; ++i) {
		EGLint gbm_format;

		if(eglGetConfigAttrib(display, configs[i],
					EGL_NATIVE_VISUAL_ID, &gbm_format) == EGL_FALSE)
			(void)fprintf(stderr, "eglGetConfigAttrib failed \n");
...
static void egl_init(uint32_t size_x, uint32_t size_y)
{
	...
	if(fd < 0)
		(void)fprintf(stderr, "couldn't open %s, skipping\n", devicenode);

	gbm = gbm_create_device(fd);
	if(gbm == NULL)
		(void)fprintf(stderr, "couldn't create gbm device \n");

	if((display = eglGetPlatformDisplay(EGL_PLATFORM_GBM_KHR, gbm, NULL)) == EGL_NO_DISPLAY)
		(void)fprintf(stderr, "eglGetPlatformDisplay failed with vendor \n");

	EGLint majorVersion;
	EGLint minorVersion;
	if(eglInitialize(display, &majorVersion, &minorVersion) == EGL_FALSE)
		(void)fprintf(stderr, "Failed to initialize EGL \n");

	if(eglBindAPI(EGL_OPENGL_ES_API) == EGL_FALSE)
		(void)fprintf(stderr, "EGL: Failed to bind OpenGL ES \n");
	...
	if((surface = eglCreatePlatformWindowSurface(display, config, gs, NULL)) == EGL_NO_SURFACE)
		(void)fprintf(stderr, "Failed to allocate surface \n");
	...
	if((context = eglCreateContext(display, config, EGL_NO_CONTEXT, contextAttribs)) == EGL_NO_CONTEXT)
		(void)fprintf(stderr, "Failed to create context \n");

	if(eglMakeCurrent(display, surface, surface, context) == EGL_FALSE)
		(void)fprintf(stderr, "Failed to make context current \n");
}
...
static int stress_gpu(const stress_args_t *args)
{
	...
	(void)stress_get_setting("gpu-size", &texsize);
	(void)stress_get_setting("gpu-upload", &uploads);

	gpu_size = size_x * size_y;
	gpu = mmap(NULL, gpu_size, PROT_READ | PROT_WRITE,
		MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	if (gpu == MAP_FAILED) {
		pr_inf("%s: cannot allocate gpu buffer of size %zu skipping stressor\n",
			args->name, gpu_size);
	...
	do{
		...
		if(glGetError() != GL_NO_ERROR)
			pr_inf("%s: failed to create texture\n", args->name);
		else break;
		inc_counter(args);
