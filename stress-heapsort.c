...
static int stress_heapsort(stress_args_t *args)
{
	...
	stress_memory_anon_name_set(data, data_size, "heapsort-data");
	...
	if (stress_signal_handler(args->name, SIGALRM, stress_heapsort_handler, &old_action) < 0) {
...
tidy:
	...
	stress_metrics_set(args, "heapsort comparisons per sec",
		rate, STRESS_METRIC_HARMONIC_MEAN);
	rate = (sorted > 0.0) ? count / sorted : 0.0;
	stress_metrics_set(args, "heapsort comparisons per item",
		rate, STRESS_METRIC_HARMONIC_MEAN);
