...
static const stress_help_t help[] = {
	...
	{ NULL, "hrtimers-adjust","adjust rate to try and maximize timer rate" },
...
static int stress_hrtimers(stress_args_t *args)
{
	...
	if (s_pids == MAP_FAILED) {
		pr_inf_skip("%s: failed to mmap %d PIDs%s, skipping stressor\n",
			args->name, PROCS_MAX, stress_memory_free_get());
		return EXIT_NO_RESOURCE;
	}
...
reap:
	...
	if (start_time >= 0.0) {
		...
		if (dt > 0.0) {
			...
			stress_metrics_set(args, "hrtimer signals per sec",
