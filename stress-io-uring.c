...
static int stress_io_uring_child(stress_args_t *args, void *context)
{
	...
	stress_memory_anon_name_set(io_uring_file.iovecs, io_uring_file.iovecs_sz, "iovecs");
	...
	for (i = 0; (i < blocks) && (file_size > 0); i++) {
		...
		stress_memory_anon_name_set(io_uring_file.iovecs[i].iov_base, block_size, "iovec-buffer");
