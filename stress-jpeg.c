	...
	const double whence)
{
	...
	double cx, cy;
	double value;
	const double third = 0.3333333333333333333;
...
static int stress_jpeg(stress_args_t *args)
{
	...
	stress_memory_anon_name_set(rgb, rgb_size, "rgb-data");
	...
	if (stress_instance_zero(args)) {
		...
		stress_memory_usage_get(args, total_size, total_size * args->instances);
	}
	...
	stress_metrics_set(args, "megapixels compressed per sec",
