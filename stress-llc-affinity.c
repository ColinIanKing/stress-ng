...
static const stress_help_t help[] = {
	...
	{ NULL, "llc-affinity-clflush",	"enable cache line flushing on memory" },
...
static int stress_llc_affinity(stress_args_t *args)
{
	...
	if (buf == MAP_FAILED) {
		pr_inf_skip("%s: mmap region of %zu bytes failed%s, errno=%d (%s), "
			"skipping stressor\n", args->name, mmap_sz,
	...
	if (stress_instance_zero(args))
		stress_memory_usage_get(args, mmap_sz, mmap_sz * args->instances);
