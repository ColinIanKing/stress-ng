...
static void stress_mlock_misc(stress_args_t *args, const size_t page_size, const bool oom_avoid)
{
	...
	VOID_RET(int, munlock((void *)stress_null_get(), ~(size_t)0));

	VOID_RET(int, shim_mlock((void *)0, 0));
	VOID_RET(int, munlock((void *)stress_null_get(), 0));
...
static int stress_mlock_child(stress_args_t *args, void *context)
{
	...
	stress_memory_anon_name_set(mappings, mappings_len, "mmap-mappings");
	...
	stress_metrics_set(args, "nanosecs per mlock call",
		rate * STRESS_DBL_NANOSECOND, STRESS_METRIC_HARMONIC_MEAN);
	if (munlock_count > 0.0) {
		rate =  munlock_duration / munlock_count;
		stress_metrics_set(args, "nanosecs per munlock call",
			rate * STRESS_DBL_NANOSECOND, STRESS_METRIC_HARMONIC_MEAN);
	}
