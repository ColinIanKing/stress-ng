...
static const stress_t stressors[] = {
	...
	STRESSOR(aio, AIO),
	STRESSOR(aio_linux, AIO_LINUX),
	STRESSOR(apparmor, APPARMOR),
	STRESSOR(atomic, ATOMIC),
	STRESSOR(bigheap, BIGHEAP),
	STRESSOR(bind_mount, BIND_MOUNT),
	STRESSOR(branch, BRANCH),
	STRESSOR(brk, BRK),
	STRESSOR(bsearch, BSEARCH),
	STRESSOR(cache, CACHE),
	STRESSOR(cap, CAP),
	STRESSOR(chdir, CHDIR),
	STRESSOR(chmod, CHMOD),
	STRESSOR(chown, CHOWN),
	STRESSOR(chroot, CHROOT),
	STRESSOR(clock, CLOCK),
	STRESSOR(clone, CLONE),
	STRESSOR(context, CONTEXT),
	STRESSOR(copy_file, COPY_FILE),
	STRESSOR(cpu, CPU),
	STRESSOR(cpu_online, CPU_ONLINE),
	STRESSOR(crypt, CRYPT),
	STRESSOR(cyclic, CYCLIC),
	STRESSOR(daemon, DAEMON),
	STRESSOR(dccp, DCCP),
	STRESSOR(dentry, DENTRY),
	STRESSOR(dev, DEV),
	STRESSOR(dev_shm, DEV_SHM),
	STRESSOR(dir, DIR),
	STRESSOR(dirdeep, DIRDEEP),
	STRESSOR(dnotify, DNOTIFY),
	STRESSOR(dup, DUP),
	STRESSOR(dynlib, DYNLIB),
	STRESSOR(efivar, EFIVAR),
	STRESSOR(enosys, ENOSYS),
	STRESSOR(epoll, EPOLL),
	STRESSOR(eventfd, EVENTFD),
	STRESSOR(exec, EXEC),
	STRESSOR(fallocate, FALLOCATE),
	STRESSOR(fanotify, FANOTIFY),
	STRESSOR(fault, FAULT),
	STRESSOR(fcntl, FCNTL),
	STRESSOR(fiemap, FIEMAP),
	STRESSOR(fifo, FIFO),
	STRESSOR(file_ioctl, FILE_IOCTL),
	STRESSOR(filename, FILENAME),
	STRESSOR(flock, FLOCK),
	STRESSOR(fork, FORK),
	STRESSOR(fp_error, FP_ERROR),
	STRESSOR(fstat, FSTAT),
	STRESSOR(full, FULL),
	STRESSOR(funccall, FUNCCALL),
	STRESSOR(futex, FUTEX),
	STRESSOR(get, GET),
	STRESSOR(getdent, GETDENT),
	STRESSOR(getrandom, GETRANDOM),
	STRESSOR(handle, HANDLE),
	STRESSOR(hdd, HDD),
	STRESSOR(heapsort, HEAPSORT),
	STRESSOR(hrtimers, HRTIMERS),
	STRESSOR(hsearch, HSEARCH),
	STRESSOR(icache, ICACHE),
	STRESSOR(icmp_flood, ICMP_FLOOD),
	STRESSOR(inode_flags, INODE_FLAGS),
	STRESSOR(inotify, INOTIFY),
	STRESSOR(iosync, IOSYNC),
	STRESSOR(iomix, IOMIX),
	STRESSOR(ioport, IOPORT),
	STRESSOR(ioprio, IOPRIO),
	STRESSOR(itimer, ITIMER),
	STRESSOR(kcmp, KCMP),
	STRESSOR(key, KEY),
	STRESSOR(kill, KILL),
	STRESSOR(klog, KLOG),
	STRESSOR(lease, LEASE),
	STRESSOR(link, LINK),
	STRESSOR(locka, LOCKA),
	STRESSOR(lockbus, LOCKBUS),
	STRESSOR(lockf, LOCKF),
	STRESSOR(lockofd, LOCKOFD),
	STRESSOR(longjmp, LONGJMP),
	STRESSOR(lsearch, LSEARCH),
	STRESSOR(madvise, MADVISE),
	STRESSOR(malloc, MALLOC),
	STRESSOR(matrix, MATRIX),
	STRESSOR(mcontend, MCONTEND),
	STRESSOR(membarrier, MEMBARRIER),
	STRESSOR(memcpy, MEMCPY),
	STRESSOR(memfd, MEMFD),
	STRESSOR(memrate, MEMRATE),
	STRESSOR(memthrash, MEMTHRASH),
	STRESSOR(mergesort, MERGESORT),
	STRESSOR(mincore, MINCORE),
	STRESSOR(mknod, MKNOD),
	STRESSOR(mlock, MLOCK),
	STRESSOR(mmap, MMAP),
	STRESSOR(mmapaddr, MMAPADDR),
	STRESSOR(mmapfixed, MMAPFIXED),
	STRESSOR(mmapfork, MMAPFORK),
	STRESSOR(mmapmany, MMAPMANY),
	STRESSOR(mq, MQ),
	STRESSOR(mremap, MREMAP),
	STRESSOR(msg, MSG),
	STRESSOR(msync, MSYNC),
	STRESSOR(netdev, NETDEV),
	STRESSOR(netlink_proc, NETLINK_PROC),
	STRESSOR(nice, NICE),
	STRESSOR(nop, NOP),
	STRESSOR(null, NULL),
	STRESSOR(numa, NUMA),
	STRESSOR(oom_pipe, OOM_PIPE),
	STRESSOR(opcode, OPCODE),
	STRESSOR(open, OPEN),
	STRESSOR(personality, PERSONALITY),
	STRESSOR(physpage, PHYSPAGE),
	STRESSOR(pipe, PIPE),
	STRESSOR(poll, POLL),
	STRESSOR(prctl, PRCTL),
	STRESSOR(procfs, PROCFS),
	STRESSOR(pthread, PTHREAD),
	STRESSOR(ptrace, PTRACE),
	STRESSOR(pty, PTY),
	STRESSOR(qsort, QSORT),
	STRESSOR(quota, QUOTA),
	STRESSOR(radixsort, RADIXSORT),
	STRESSOR(rawdev, RAWDEV),
	STRESSOR(rdrand, RDRAND),
	STRESSOR(readahead, READAHEAD),
	STRESSOR(remap_file_pages, REMAP_FILE_PAGES),
	STRESSOR(rename, RENAME),
	STRESSOR(resources, RESOURCES),
	STRESSOR(revio, REVIO),
	STRESSOR(rlimit, RLIMIT),
	STRESSOR(rmap, RMAP),
	STRESSOR(rtc, RTC),
	STRESSOR(schedpolicy, SCHEDPOLICY),
	STRESSOR(sctp, SCTP),
	STRESSOR(seal, SEAL),
	STRESSOR(seccomp, SECCOMP),
	STRESSOR(seek, SEEK),
	STRESSOR(semaphore_posix, SEMAPHORE_POSIX),
	STRESSOR(semaphore_sysv, SEMAPHORE_SYSV),
	STRESSOR(sendfile, SENDFILE),
	STRESSOR(set, SET),
	STRESSOR(shm_posix, SHM_POSIX),
	STRESSOR(shm_sysv, SHM_SYSV),
	STRESSOR(sigfd, SIGFD),
	STRESSOR(sigfpe, SIGFPE),
	STRESSOR(sigio, SIGIO),
	STRESSOR(sigpending, SIGPENDING),
	STRESSOR(sigpipe, SIGPIPE),
	STRESSOR(sigqueue, SIGQUEUE),
	STRESSOR(sigrt, SIGRT),
	STRESSOR(sigsegv, SIGSEGV),
	STRESSOR(sigsuspend, SIGSUSPEND),
	STRESSOR(sleep, SLEEP),
	STRESSOR(socket, SOCKET),
	STRESSOR(socket_diag, SOCKET_DIAG),
	STRESSOR(socket_fd, SOCKET_FD),
	STRESSOR(socket_pair, SOCKET_PAIR),
	STRESSOR(softlockup, SOFTLOCKUP),
	STRESSOR(spawn, SPAWN),
	STRESSOR(splice, SPLICE),
	STRESSOR(stack, STACK),
	STRESSOR(stackmmap, STACKMMAP),
	STRESSOR(str, STR),
	STRESSOR(stream, STREAM),
	STRESSOR(swap, SWAP),
	STRESSOR(switch, SWITCH),
	STRESSOR(symlink, SYMLINK),
	STRESSOR(sync_file, SYNC_FILE),
	STRESSOR(sysbadaddr, SYSBADADDR),
	STRESSOR(sysinfo, SYSINFO),
	STRESSOR(sysfs, SYSFS),
	STRESSOR(tee, TEE),
	STRESSOR(timer, TIMER),
	STRESSOR(timerfd, TIMERFD),
	STRESSOR(tlb_shootdown, TLB_SHOOTDOWN),
	STRESSOR(tmpfs, TMPFS),
	STRESSOR(tree, TREE),
	STRESSOR(tsc, TSC),
	STRESSOR(tsearch, TSEARCH),
	STRESSOR(udp, UDP),
	STRESSOR(udp_flood, UDP_FLOOD),
	STRESSOR(unshare, UNSHARE),
	STRESSOR(urandom, URANDOM),
	STRESSOR(userfaultfd, USERFAULTFD),
	STRESSOR(utime, UTIME),
	STRESSOR(vecmath, VECMATH),
	STRESSOR(vfork, VFORK),
	STRESSOR(vforkmany, VFORKMANY),
	STRESSOR(vm, VM),
	STRESSOR(vm_addr, VM_ADDR),
	STRESSOR(vm_rw, VM_RW),
