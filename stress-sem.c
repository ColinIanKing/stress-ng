...
static void stress_sem_init(const uint32_t instances)
{
	...
	stress_memory_anon_name_set(sem_global, sizeof(*sem_global), "shared-semaphore");
...
static int stress_sem(stress_args_t *args)
{
	...
	if (duration > 0.0) {
		stress_metrics_set(args, "sem_trywait calls per sec",
			trywait_count / duration, STRESS_METRIC_HARMONIC_MEAN);
		stress_metrics_set(args, "sem_timedwait calls per sec",
			timedwait_count / duration, STRESS_METRIC_HARMONIC_MEAN);
		stress_metrics_set(args, "sem_wait calls per sec",
