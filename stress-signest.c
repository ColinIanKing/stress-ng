...
static void MLOCKED_TEXT stress_signest_handler(int signum)
{
	...
	if (UNLIKELY(run_time > (double)g_opt_timeout)) {
		...
		stress_signal_siglongjmp_flag(signum, jmp_env, 1, &jmp_env_ok);
	}
...
static int stress_signest(stress_args_t *args)
{
	...
	stress_memory_anon_name_set(altstack, altstack_size, "altstack");

	if (stress_stack_sigalt(altstack, altstack_size) < 0) {
	...
	signal_info.altstack = (intptr_t)(stress_stack_direction() > 0 ?
...
finish:
	...
	stress_metrics_set(args, "nanosec to handle a signal",
