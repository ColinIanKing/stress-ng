...
static int stress_sockmany(stress_args_t *args)
{
	...
	sockmany_port = stress_net_port_wraparound(sockmany_port);
	reserved_port = stress_net_reserve_ports(args, sockmany_port, sockmany_port);
	if (reserved_port < 0) {
	...
	if (sock_fds == MAP_FAILED) {
		pr_inf_skip("%s: mmap %zu byte shared memory failed%s, errno=%d (%s), "
			"skipping stressor\n",
