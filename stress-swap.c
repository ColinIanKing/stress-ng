...
static int stress_swap_child(stress_args_t *args, void *context)
{
	...
	stress_memory_anon_name_set(page, page_size, "swap-page");
	...
	if (stress_instance_zero(args)) {
		...
		stress_memory_usage_get(args, size, size * args->instances);
	}
	...
	stress_metrics_set(args, "pages swapped out per second", rate, STRESS_METRIC_GEOMETRIC_MEAN);
