...
static int OPTIMIZE3 stress_udp_flood(stress_args_t *args)
{
	...
	rate = (total_count > 0) ? ((double)(total_count - sendto_failed) / (double)total_count) * 100.0 : 0.0;
	stress_metrics_set(args, "% sendto calls succeeded", rate, STRESS_METRIC_HARMONIC_MEAN);
