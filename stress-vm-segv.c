...
{
	size_t len = ~(size_t)0;
	uint8_t *addr = (uint8_t *)stress_memory_address_align((void *)vm_unmap_child, page_size);
...
static NOINLINE void vm_unmap_self(const size_t page_size)
{
	 ...
	 */
#else
	uint8_t *addr = (uint8_t *)stress_memory_address_align((void *)vm_unmap_self, page_size);
...
{
	uint32_t stackvar = 0;
	uint8_t *addr = (uint8_t *)stress_memory_address_align((void *)&stackvar, page_size);
